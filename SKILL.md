...
### 规则 6：逐项询问，不问多余问题
- **一次只问一项**，不要把所有缺失项堆在一段话里同时问
- 用户回答一项后，再问下一项
- 除非用户在一条消息里一次性回答了多项，才批量确认
- 每项询问要有**固定提问格式**（见下方"逐项询问固定话术"）
...
### 方式三：只说触发词 + 逐步补充（一次只问一项）

> **你**：生成utm链接
> **Agent**：目标链接是哪个页面？
> **你**：shuishouac.com
> **Agent**：好的。活动名称：用词库里已有的标准名（比如 `black-friday` 黑五、`spring-sale` 春季大促），还是你自己定义一个固定名称？
> **你**：标准名，黑色星期五
> **Agent**：好的，活动名称使用 `black-friday`，后续同活动请保持统一，可以吗？
> **你**：可以
> **Agent**：通过什么方式触达？广告投放、达人合作、邮件营销？
> **你**：Instagram 达人 Jessica
> **Agent**：好的，媒介 `influencer`，来源 `instagram`，达人 `creator-jessica`。发布日期是哪天？
> **你**：7月11号
> **Agent**：需要区分不同发布场景吗？比如 Story、Post、Bio 各生成一个链接，还是统一用一个？
> **你**：统一一个就行
> **Agent**：（生成完整链接）
...
**第三步：信息缺失时逐项询问（一次只问一项）**

当扫描上下文后发现某参数缺失时，**一次只问一项**，用户回答后确认再问下一项。

### 逐项询问固定话术

按以下顺序缺失哪项就问哪项，每项使用固定话术，不要合并：
...
用户回答后确认：
> "好的，目标链接：`https://shuishouac.com/sale`"

---

**② campaign 缺失**
> "活动名称：你是用词库里已有的标准活动名，还是自己定义一个固定名称？"
> - 标准：`black-friday`（黑五）、`spring-sale`（春季大促）、`member-day`（会员日）……
> - 自定义：你告诉我，这次活动叫什么？

用户选择后确认：
> - 标准名 → "活动名称使用 `black-friday`，后续同活动请保持统一，可以吗？"
> - 自定义 → "活动名称为 `summer-sale-2026`，后续统一用这个，可以吗？"

---

**③ medium 缺失**
> "通过什么方式触达？广告投放、达人合作、邮件营销，还是其他？"

---

**④ source 缺失**
> "在哪个平台发布？Instagram、TikTok、Facebook，还是其他？"
...
**⑦ 场景后缀缺失**
> "需要区分不同发布场景吗？比如 Story、Post、Bio 各生成一个链接，还是统一用一个链接就行？"
> - 统一 → 默认 `launch`
> - 分场景 → 说明有哪些场景（如 Story / Post / Reels / Bio）
