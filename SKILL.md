...
- 生成时必须先询问用户：**"用参考列表里的名称（如 `black-friday`、`spring-sale`），还是你自己定义一个固定名称？"**
- 选择标准名称 → 从参考列表选取，确认用户同意
- 选择自定义名称 → 用户输入 → 转小写+连字符 → 确认一致性：**"campaign 名称使用 `xxx`，后续同活动请保持统一，可以吗？"**
...
| 选项 | ① `标准词库` — 从已有活动名选取（black-friday、spring-sale 等） ② `自定义` — 我自己定义一个固定名称 |
...
| header | `选择活动名` |
| question | `选择最匹配的活动名称` |
| 选项 | ① `black-friday` — 黑色星期五 ② `spring-sale` — 春季大促 ③ `christmas` — 圣诞节 ④ `其他` — 从词库选其他 |
...
|------|------|
| header | `自定义活动名` |
| question | `输入你的活动名称（英文小写，空格会自动转连字符）` |
| 选项 | ① `summer-sale-2026` — 示例 ② `其他` — 手动输入 |

---

**第 3 弹：确认 campaign 名称（必须单独弹）**
