...
### 规则 2：campaign 自动匹配或询问确认
- 用户说了活动关键词（如"黑色星期五"、"黑五"） → 自动匹配词库中最接近的标准名 `black-friday` → 弹窗确认：**"campaign 名称使用 black-friday，可以吗？"**
- 匹配不到或用户自定义 → 弹窗让用户选择：用词库标准名，还是自己定义固定名称
- **用户已确认过的 campaign，后续同活动不再重复问**
...
### 规则 4：term 始终生成 + 日期优先 + 场景后缀
utm_term 是**必填参数**，永远不允许省略。
- 先让用户选择日期 → 自动生成 `YYYYMMDD`
- 后缀：**默认 `launch`**，有场景区分时替换（如 `ig-story`、`ig-post`）
...
### 规则 6：用弹窗对话框收集信息，不用聊天式一问一答
- 使用 **AskUserQuestion 工具**弹出选择对话框，不要在聊天里发文字提问
- 每次弹窗 1-3 个问题，不要一次弹太多
- 每个问题提供 2-4 个预设选项 + 系统自带 Other（自由输入）
- 需要确认的步骤（如 campaign 名称确认）单独弹窗
- 用户已在上下文中提供的信息，不要再弹窗问
- 确认类弹窗（第 2 弹、第 4 弹）**只放 1 个确认预设**，第二个预设放一个有意义的具体备选（见 Rule 10）

### 规则 7：campaign 名称必须先选来源
- **每次生成必须先问**：用标准词库的活动名，还是自定义固定名称？
- 标准词库 → 展示相关分类供选择（如节日类 `black-friday`、`christmas`）
- 自定义 → 用户输入 → 格式化 → 确认一致性
- 用户选择后，后续同活动不再重复问

### 规则 9：上下文已有关键信息只需确认，不重新输入
- 用户一句话指令中明确提到的信息：活动名、平台、达人名、日期 → **自动提取，弹窗只做确认**
- 例如用户说"黑色星期五，Instagram 达人 Jessica"：
  - ✅ 自动匹配 `black-friday` → 弹窗确认"是这个词吗？"
  - ✅ 自动提取 `influencer` + `instagram` + `creator-jessica` → 弹窗确认"渠道信息这样对吗？"
  - ❌ 不要问"用标准词库还是自定义？"（已经匹配到了）
  - ❌ 不要问"在哪个平台？"（用户已经说了 Instagram）
- 只有当上下文**找不到**对应信息时，才弹窗让用户补充

### 规则 10：确认框只放「确认」预设，修改全部走 Other
- 所有确认类弹窗（campaign 确认、渠道确认等）**只放 1 个确认预设选项**
- **第二个预设必须是一个有意义的具体备选**（如另一个活动名或渠道），不能放「换一个」「需要修改」「重新输入」——这些选了还要再弹一框，且与系统 Other 重复
- 用户需要其他修改 → 直接用 **系统 Other** 输入新值，同一框搞定
- 问题文本末尾统一加提示：`（如需修改，选下方 Other 手动输入）`
- 示例：
  - ✅ `campaign 使用 black-friday，可以吗？` → 选项：`可以` + `用 summer-sale` + Other(输入任意名称)
  - ❌ `campaign 使用 black-friday，可以吗？` → 选项：`可以` / `换一个`（换一个又弹一框）
- 当用户选择"全场景"或"多个场景"时，生成**多条**链接
- 每条链接必须标注**用在哪里**（Story / Post / Reels / Bio）
- 输出格式见下方"多场景输出格式"
...
**格式**: `YYYYMMDD` + 后缀（默认 `launch`，可自定义）

**生成逻辑（优先级从上到下）**：

1. 用户自定义完整 term → 直接使用
2. 用户指定日期 + 自定义后缀 → `20260713ig-story`
3. 用户指定日期 + 默认后缀 → `20260713launch`
4. 都没指定 → 今天日期 + `launch`（`20260713launch`）

**规则**：
- 日期部分：8 位数字（如 `20260713`）
- 后缀部分：
  - **默认** `launch`（表示常规发布/上线）
  - **有场景区分时**替换为具体后缀，如 `ig-story`、`ig-post`、`bio`、`fb-ad`
  - 用户也可显式去掉后缀（仅日期）
- 全部小写，无空格
- **content 已标识的信息，term 不重复；term 已标识的信息，content 不重复**

**示例**：
- 2026年7月13日发布，默认 → `20260713launch`
- 2026年7月13日 IG Story 区分 → `20260713ig-story`
- 2026年7月13日 IG Post 区分 → `20260713ig-post`
- 2026年7月13日 Bio 链接区分 → `20260713bio`
- 显式无后缀 → `20260713`
- 完全自定义 → `q3-launch-2026`
...
### 方式三：只说触发词 + 逐步补充（弹窗对话框）

用户只说触发词，Agent 通过 **弹窗对话框（AskUserQuestion）** 让用户选择或补充信息。每次 1-3 个问题，分批弹出，不要一次太多。

**示例：用户说"生成utm链接，黑色星期五，Instagram 达人 Jessica"**

Agent 从指令中自动提取：
- `黑色星期五` → 匹配词库 `black-friday`
- `Instagram 达人` → `influencer` + `instagram`
- `Jessica` → `creator-jessica`
...
> ┌─ 目标链接 ──────────────────────────┐
> │ 目标链接是哪个页面？                  │
> │ ○ https://shuishouac.com             │
> │ ○ https://shuishouac.com/bag         │
> │ ── Other 可手动输入 ──               │
> └──────────────────────────────────────┘
>
> **用户**：选择 `https://shuishouac.com`
>
> **Agent**（自动匹配到 black-friday，弹窗确认）：
> ┌─ 确认活动名 ────────────────────────┐
> │ campaign 使用 black-friday，可以吗？  │
> │ ○ 可以，就用这个                     │
> │ ○ 用 summer-sale（春季大促）         │
> │ ── Other 可输入任意名称 ──           │
> └──────────────────────────────────────┘
...
> **用户**：选择 `可以`
>
> **Agent**（已提取渠道信息，弹窗批量确认）：
> ┌─ 渠道确认 ──────────────────────────┐
> │ 渠道信息：influencer + instagram     │
> │ 达人：creator-jessica，对吗？        │
> │ ○ 对的，没问题                      │
> │ ○ 改成 TikTok 达人                   │
> │ ── Other 可输入修改 ──               │
> └──────────────────────────────────────┘
...
**示例对比：用户只说"生成utm链接"（无关键词）**

Agent 无法提取任何信息，从零开始逐项询问：

> **Agent**：目标链接是哪个页面？ → **用户回答**
> **Agent**：活动名用词库标准名还是自定义？ → **用户选择**
> **Agent**：通过什么方式触达？ → **用户回答**
> **Agent**：在哪个平台？ → **用户回答**
> **Agent**：达人/素材叫什么？ → **用户回答**
> **Agent**：发布日期？ → **用户回答**
...
> **Agent**：（生成链接）
>
> > ⚠️ 每个步骤都使用弹窗对话框，且确认类步骤**选择即确认**、修改走 Other，不多弹一框。
...
如果当前对话中已经包含了必要的信息（URL、活动类型、渠道、日期等），直接提取，**不需要再弹窗询问**。

但日期必须用户明确提供——上下文没提到日期时，弹窗询问，不能默认用当天。
...
| `campaign` | ①用户提了活动名 ②上下文推断活动类型 ③弹窗询问 |
| `medium` | ①上下文触达方式 ②弹窗询问 |
| `source` | ①上下文平台/渠道 ②弹窗询问 |
| `content` | ①上下文达人/素材名 ②弹窗询问 |
| `term` （日期） | **①用户显式提供 ②弹窗询问（不使用默认当天）** |
| `term` （后缀） | ①用户指定 ②默认 `launch` ③弹窗询问场景区分 |

**第三步：用弹窗对话框收集缺失信息**

使用 **AskUserQuestion 工具**弹出对话框，替代聊天式一问一答。遵循以下原则：

1. **每次 1-3 个问题**，不要一次性弹出所有问题
2. **先弹最关键的**：先问 URL 和 campaign，再问渠道细节，最后问日期和场景
3. **提供预设选项** + 用户可"其他"手动输入
4. **需要确认的步骤**（如 campaign 确认）单独弹窗
5. **不要合并两个不同维度的问题**：medium 和 source 可以合并弹一个窗，但 campaign 确认必须单独弹

### 弹窗对话框分步模板

按顺序依次弹出（缺失项才弹，已从上下文提取到的不弹）：
...
| header | `目标链接` |
| question | `目标链接是哪个页面？` |
| 选项 ① | `shuishouac.com` — 首页 |
| 选项 ② | `shuishouac.com/bag` — 新品包包页（示例） |
| 其他 | 系统 Other → 用户手动输入其他链接 |

> ⚠️ **不要**放「其他页面」作为预设选项——系统自带 Other 已支持自由输入，放两个"其他"会让用户困惑。

---

**第 2 弹：活动名称（分情况）**

**情况 A：用户指令中提到了活动关键词（如"黑色星期五"、"黑五"、"春季大促"）**
...
|------|------|
| header | `确认活动名` |
| question | `campaign 名称使用「black-friday」，后续同一活动请保持统一，可以吗？` |
| 选项 ① | `可以，就用这个` — 确认使用 |
| 选项 ② | `用 summer-sale` — 春季大促（或其他常见备选） |
| 其他 | 系统 Other → 用户直接输入任意 campaign 名称 |

> 选项 ② 选一个最常见、最可能被用户需要的备选活动名即可，不追求覆盖全。用户要其他名称 → Other 手动输入。
...
|------|------|
| header | `活动名称` |
| question | `活动名称：用标准词库的活动名，还是自定义一个固定名称？` |
| 选项 ① | `标准词库` — 从已有活动名选取 |
| 选项 ② | `自定义` — 我自己定义一个固定名称 |

> ⚠️ 不要放"其他"选项，系统 Other 已支持自由输入。

**如果选「标准词库」→ 弹出选择，选择即确认**（不再额外弹确认框）：

| 字段 | 内容 |
|------|------|
| header | `选择活动名` |
| question | `选择最匹配的活动名称（选中的名称将直接作为 campaign 名称）` |
| 选项 | ① `black-friday` — 黑色星期五 ② `spring-sale` — 春季大促 ③ `summer-sale` — 夏季促销 ④ `new-product` — 新品发布 |
| 其他 | 系统 Other → 用户可输入其他活动名 |

> **关键**：用户从列表中选择后，**直接确定**，不再弹"确认使用 xxx 吗？"的第二次确认框。如果用户选错了，直接在 Other 里重新输入即可。
...
|------|------|
| header | `自定义活动名` |
| question | `输入你的活动名称（英文小写，空格自动转连字符，后续同活动请保持统一）` |
| 其他 | 系统 Other → 手动输入 |

> 自定义名称不需要再弹确认框。Agent 格式化后，在最终生成时附带一致性提醒即可。

> ⚠️ **关键原则**：如果上下文已匹配到活动名，**直接跳到确认步骤**，不要问"标准还是自定义"。
...
如"Instagram 达人 Jessica" → 自动提取 `influencer` + `instagram` + `creator-jessica`，弹窗批量确认：

| 字段 | 内容 |
|------|------|
| header | `渠道确认` |
| question | `渠道信息：influencer（达人营销）+ instagram（Instagram）+ creator-jessica（达人Jessica)，对吗？` |
| 选项 ① | `对的，没问题` |
| 选项 ② | `改成 TikTok 达人` — 另一个常见渠道的快捷切换 |
| 其他 | 系统 Other → 用户直接输入修改，如「改成 tiktok 达人 carry」

**情况 B：用户没说渠道信息**

先弹出 medium（选项来自词库标准 medium 列表，**不放「其他」**，由系统 Other 处理自由输入）：

| 字段 | 内容 |
|------|------|
| header | `触达方式` |
| question | `通过什么方式触达？` |
| 选项 | ① `达人合作` — influencer ② `广告投放` — paid-social ③ `邮件营销` — email |
| 其他 | 系统 Other → 用户可输入任意 medium（如 affiliate, qr, video 等） |
...
| header | `发布平台` |
| question | `在哪个平台发布？` |
| 选项 | ① `Instagram` ② `TikTok` ③ `Facebook` |
| 其他 | 系统 Other → 用户可输入任意 source（如 youtube, linkedin, x 等） |
...
| header | `发布日期` |
| question | `发布日期是哪天？` |
| 选项 | ① `今天` — 2026-07-14 ② `明天` — 2026-07-15 |
| 其他 | 系统自带「Other」手动输入，用户可直接填「7月11日」「2026.7.11」「0711」等 |

> ⚠️ **不要**把"其他日期"做成选项——选了还会再弹一次。直接靠系统自带的 Other 手动输入即可，一轮搞定。
...
|------|------|
| header | `发布场景` |
| question | `需要区分不同发布形式吗？` |
| 选项 | ① `统一用一个链接` — 默认 launch 后缀 ② `全场景` — Story+Post+Reels+Bio 各一个 ③ `自定义场景` — 手动说明哪些场景 |

---

**第 8 弹（可选）：自定义后缀**

如果用户在第 7 弹选了"自定义场景"，弹出：

| 字段 | 内容 |
|------|------|
| header | `自定义后缀` |
| question | `term 后缀用什么？例如 ig-story、ig-post、bio。留空则只有日期` |
| 选项 | ① `ig-story` — Story 链接 ② `ig-post` — Post 链接 ③ `bio` — Bio 链接 |
| 其他 | 系统 Other → 手动输入 |
...
① 📱 Instagram Story
┃ https://shuishouac.com/?utm_campaign=...&utm_content=creator-jessica&utm_term=20260713ig-story
┃ 用  途：Jessica 的 Instagram Story 贴纸/链接

② 📄 Instagram Post
┃ https://shuishouac.com/?utm_campaign=...&utm_content=creator-jessica&utm_term=20260713ig-post
┃ 用  途：Jessica 的 Feed 帖文

③ 🎬 Instagram Reels
┃ https://shuishouac.com/?utm_campaign=...&utm_content=creator-jessica&utm_term=20260713reels
┃ 用  途：Jessica 的 Reels 视频

④ 🔗 Bio 简介
┃ https://shuishouac.com/?utm_campaign=...&utm_content=creator-jessica&utm_term=20260713bio
┃ 用  途：Jessica Instagram 主页简介链接
...
| utm_medium | {medium} | 营销媒介 |
| utm_source | {source} | 流量来源 |
| utm_content | {content} | 内容标识（各场景共用） |

📋 区分参数（各场景不同）

| 场景 | utm_term | 用途 |
|------|----------|------|
| Story | `20260713ig-story` | Instagram Story |
| Post | `20260713ig-post` | Feed 帖文 |
| Reels | `20260713reels` | Reels 视频 |
| Bio | `20260713bio` | 主页简介 |
```
