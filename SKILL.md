...
### 规则 4：term 始终生成 + 日期优先 + 可自定义后缀
utm_term 是**必填参数**，永远不允许省略。
- 先让用户选择日期 → 自动生成 `YYYYMMDD`
- 再让用户自定义后缀（默认 `launch`） → `YYYYMMDD{suffix}`
- 用户也可以输入完全自定义的 term
- 如果用户都没指定 → 默认 `当天日期 + launch`
...
1. 用户自定义完整 term → 直接使用
2. 用户指定日期 + 自定义后缀 → `20260713fb-ad`
3. 用户指定日期 + 默认后缀 → `20260713launch`
4. 都没指定 → `当天日期 + launch`

**规则**：
- 日期部分：8 位数字（如 `20260713`）
- 后缀部分：**完全自定义**，不限于 `launch`
  - 默认后缀：`launch`
  - 常用后缀示例：`launch`、`fb-ad`、`ig-story`、`vip`、`email-welcome`、`flash-sale`
- 全部小写，无空格

**示例**：
- 2026年7月13日发布，默认 → `20260713launch`
- 2026年7月13日发帖，标记为 ig 故事 → `20260713ig-story`
- 2026年7月11日发布，自定义 → `20260711launch`
- 用户完全自定义 → `q3-launch-2026`

> 💡 **后缀建议**：用简短的内容标识区分同一活动不同批次的发布，方便在分析工具中筛选。
