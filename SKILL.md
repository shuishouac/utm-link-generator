...
- 所有确认类弹窗（campaign 确认、渠道确认等）**只放一个「确认」预设选项**
- 不要放「换一个」「需要修改」「重新输入」——这些选了还要再弹一框，且与系统 Other 重复
- 用户要修改 → 直接用 **系统 Other** 输入新值，同一框搞定
- 示例：
  - ✅ `campaign 使用 black-friday，可以吗？` → 选项：`可以` + Other(输入新名称)
...
| 选项 | ① `可以，就用这个` — 确认使用 |
| 其他 | 系统 Other → 用户直接输入新 campaign 名称 |
...
| 选项 | ① `标准词库` — 从已有活动名选取 ② `自定义` — 我自己定义一个固定名称 |
...
| header | `选择活动名` |
| question | `选择最匹配的活动名称` |
| 选项 | ① `black-friday` — 黑色星期五 ② `spring-sale` — 春季大促 |
| 其他 | 系统 Other → 用户可输入其他活动名（如 member-day, christmas 等） |
...
|------|------|
| header | `确认活动名` |
| question | `campaign 名称使用「xxx」，后续同一活动请保持统一，可以吗？` |
| 选项 | ① `可以，就用这个` |
| 其他 | 系统 Other → 用户直接输入新名称 |
...
|------|------|
| header | `自定义活动名` |
| question | `输入你的活动名称（英文小写，空格自动转连字符）` |
| 选项 | ① 手动输入 |
...
|------|------|
| header | `确认活动名` |
| question | `campaign 名称使用「xxx」，后续统一用这个，可以吗？` |
| 选项 | ① `可以` |
| 其他 | 系统 Other → 重新输入 |
